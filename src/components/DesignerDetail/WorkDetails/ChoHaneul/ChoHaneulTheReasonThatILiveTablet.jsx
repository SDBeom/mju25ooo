...
import '../../styles/works/ChoHaneulTheReasonThatILive/ChoHaneulTheReasonThatILiveTablet.css';
...
/**
 * 조하늘 - The Reason that I Live (작품2) Tablet 버전
 * Figma 디자인 기반 구현 (800px)
 */
const ChoHaneulTheReasonThatILiveTablet = ({ work, designer, ctas }) => {
  if (!work || !designer) {
    return null;
  }

  return (
    <div className="work-detail work-detail--cho-haneul-the-reason-that-i-live-tablet">
      {/* Hero Section */}
      <section className="work-detail__section work-detail__hero work-detail__hero--cho-haneul-tablet">
        <div className="work-detail__hero-content work-detail__hero-content--cho-haneul-tablet">
          <div className="work-detail__text-group">
            <h2 className="work-detail__title work-detail__title--cho-haneul-tablet">
              The Reason that I Live
            </h2>
            <p className="work-detail__lead work-detail__lead--cho-haneul-tablet">
              {work.summary || '히키코모리의 삶를 살아가던 코모리가 친구의 위로와 응원을 통해 다시 세상으로 한걸음 내딛는 이야기'}
            </p>
          </div>
          {ctas && Array.isArray(ctas) && ctas.length > 0 && (
            <div className="work-detail__ctas work-detail__ctas--cho-haneul-tablet">
              {ctas.map(({ label, onClick, variant = 'primary' }) => (
                <button
                  key={label}
                  type="button"
                  className={`work-detail__cta work-detail__cta--${variant === 'secondary' ? 'secondary' : 'primary'} work-detail__cta--cho-haneul-tablet`}
                  onClick={onClick}
                >
                  {label}
                </button>
        ...
        <div className="work-detail__hero-image-wrapper work-detail__hero-image-wrapper--cho-haneul-tablet">
          <img
      ...
      {/* Text Section 1 */}
      <section className="work-detail__section work-detail__text-section work-detail__text-section--cho-haneul-tablet">
        ...
        <div className="work-detail__text-content">
          <p>{`어두운 방 안, 멈춰있는 시간 속의 코모리.
구르미의 따뜻한 손길이 닿는 순간, 멈춰 있던 시간은 다시 흐르기 시작한다.

사라져가던 마음에서, 다시 피어나는 소망으로.
"You are the reason that I live"

현대 사회에서 우리는 끊임없이 연결되어 있지만, 역설적으로 동시에 점점 더 외로워지고 있다.
관계 속에서 위로받지 못한 감정은 서서히 마음을 닫게 만들고, 결국 자신을 잃게 만든다.

<The Reason that I Live> 는 '진정한 위로'는 무엇인가에
대한 질문을 던진다.
누군가의 손길, 한마디의 말, 작은 관심이
멈춰 있던 마음을 다시 움직이게 만들 수 있다는 믿음에서 시작된 작업이다.
...
이 이야기가 누군가의 닫힌 마음에 작은 온기로 닿을 수 있기를 바란다.`}</p>
      ...
      {/* Text Row Section */}
      <ul className="work-detail__section work-detail__text-blocks work-detail__text-blocks--cho-haneul-tablet">
      ...
      <ul className="work-detail__section work-detail__cards work-detail__cards--cho-haneul-tablet">
        <li className="work-detail__card work-detail__card--cho-haneul-tablet-1">
        ...
        <li className="work-detail__card work-detail__card--cho-haneul-tablet-2">
          <div className="work-detail__card-image-wrapper">
            <img
        ...
        <li className="work-detail__card work-detail__card--cho-haneul-tablet-3">
          <div className="work-detail__card-image-wrapper">
            <img
          ...
          <div className="work-detail__card-text">
            ...
            <p>{`따뜻한 마음을 지닌, 희망과 위로를 전하는 구름 같은 친구이다.
코모리의 소중한 친구로서, 불안 속에 있을 때 살며시 다가와 곁을 지키며 위로가 되어주었다.`}</p>
          </div>
        </li>
      </ul>

      {/* Style Frame Section 1 */}
      <section className="work-detail__section work-detail__feature work-detail__feature--cho-haneul-tablet">
...
export default ChoHaneulTheReasonThatILiveTablet;
