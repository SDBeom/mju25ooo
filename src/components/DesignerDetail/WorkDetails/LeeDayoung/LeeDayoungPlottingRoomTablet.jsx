...
조합으로 모여 자신의 작품과 
글쓰기 과정, 그리고 좋아하는 소재에 
대해 깊이 이야기를 나누며, 한 회차의
마지막에는 즉석에서 새로운 이야기를 
창작해 선보이는 가상의 토크 예능 
<플롯팅룸>의 타이틀 시퀀스를 제작한 프로젝트이다.`}
            </p>
          </div>
      ...
      <section className="work-detail__section work-detail__text-section work-detail__text-section--leedayoung-plottingroom-tablet">
        ...
        <div className="work-detail__text-content">
          <p>{`'플롯팅룸(Plotting Room)'은 Plot-ting: 이야기를 짜는 / Float-ing: 이야기가 떠다니는 / 
Flirt-ing: 이야기로 사람을 매료시키는 방이라는 다층적 의미를 담고 있다. 

이러한 제목의 의미와 프로그램의 콘셉트를 시각적으로 표현하기 위해, 다양한 시대와 방식으로 
이야기를 만들어가는 작가들의 모습을 그리고, 그들이 써 내려간 이야기들이 실사처럼 살아 움직이는 
장면을 연출하였다. 작가들의 소통과 창작의 순간들이 모여 또 하나의 서사를 이루고, 그 서사로 방이 채워지는 과정을 통해 프로그램의 주제를 완성하고자 하였다.

전반적인 장면은 2D로 구성하여 서정적인 분위기를 구현하였다. 평면적인 화면에서도 밀도가 
느껴지도록 그레인 그라데이션, 양감, 텍스처링에 중점을 두었다. 타이틀 레터링은 토크 예능의 
경쾌함을 담되, 전체 시퀀스의 서정적인 톤과 조화를 이루며 프로그램의 개성을 시각적으로 
드러내고자 하였다.`}</p>
        </div>
      </section>
